...
describe('SalesImportProcessor', () => {
  ...
  it('should process the POS Excel file successfully, map rows, and write to ledger', async () => {
    ...
    recipeService.resolveRecipesByPosStrings.mockResolvedValue([
      { id: '1' as import('@synculariti/types').MenuItemMappingId, restaurantId: restaurantId as import('@synculariti/types').RestaurantId, rawExcelString: 'BLUE HEAVEN COMBO', recipeId: 'recipe-1' as import('@synculariti/types').RecipeId, createdAt: 'time' },
      { id: '2' as import('@synculariti/types').MenuItemMappingId, restaurantId: restaurantId as import('@synculariti/types').RestaurantId, rawExcelString: 'CHEESY SMASH', recipeId: 'recipe-2' as import('@synculariti/types').RecipeId, createdAt: 'time' },
    ]);

    // Mock BOM expansion
    recipeService.expandBOM.mockImplementation(async (recipeId, quantity) => {
      if (recipeId === 'recipe-1') {
        return [{ itemId: 'item-bun' as import('@synculariti/types').ItemId, consumedQty: quantity * 1 }, { itemId: 'item-patty' as import('@synculariti/types').ItemId, consumedQty: quantity * 1 }];
      }
      if (recipeId === 'recipe-2') {
        return [{ itemId: 'item-cheese' as import('@synculariti/types').ItemId, consumedQty: quantity * 2 }];
      }
      return [];
    });
