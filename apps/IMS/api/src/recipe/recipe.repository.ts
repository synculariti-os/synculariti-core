...
import { UpdateRecipeDto } from '@synculariti/validators';
...
export class RecipeRepository implements IRecipeRepository {
  ...
  async findByRecipeName(recipeName: string): Promise<Recipe | null> {
    const row = await this.db
      .selectFrom('recipes')
      .selectAll()
  ...
  async resolveByPosString(restaurantId: RestaurantId, rawString: string): Promise<Recipe | null> {
    const row = await this.db
      ...
      .select([
        ...
        'recipes.price_eur as price_eur',
        'recipes.vat_rate as vat_rate',
    ...
    return await this.db.transaction().execute(async (trx) => {
      const [recipe] = await trx
        ...
        .values({
          ...
          recipe_name: command.recipeName,
          yield_quantity: command.yieldQuantity,
          price_eur: command.priceEur ?? null,
          vat_rate: command.vatRate ?? null,
  ...
  async update(recipeId: RecipeId, dto: UpdateRecipeDto): Promise<Recipe> {
    return await this.db.transaction().execute(async (trx) => {
      ...
      if (dto.priceEur !== undefined) {
        updateData['price_eur'] = dto.priceEur;
      }
      if (dto.vatRate !== undefined) {
        updateData['vat_rate'] = dto.vatRate;
      }
  ...
  async upsertMapping(
    ...
    const id = uuidv4() as MenuItemMappingId;
    const trimmed = rawString.trim();
    await sql`
      INSERT INTO menu_item_mappings (id, restaurant_id, raw_excel_string, recipe_id)
      VALUES (${id}::uuid, ${restaurantId}::uuid, ${trimmed}, ${recipeId}::uuid)
      ON CONFLICT (restaurant_id, raw_excel_string) WHERE deleted_at IS NULL
      DO UPDATE SET recipe_id = ${recipeId}::uuid
    `.execute(this.db);
  }
