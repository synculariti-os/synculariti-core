...
import { Database, RestaurantId, ItemId, StockLevel, asItemId, InventoryLedgerEntry, LedgerEntryId } from '@synculariti/types';
...
export class LedgerRepository implements ILedgerRepository {
  ...
  async getLedgerEntries(restaurantId: RestaurantId, limit: number, offset: number): Promise<InventoryLedgerEntry[]> {
    ...
    return rows.map(r => ({
      ...
      baseUom: r.inventory_uom as string,
    }));
  }
}
