...
export class ItemRepository implements IItemRepository {
  ...
  async findById(itemId: ItemId, restaurantId: RestaurantId): Promise<ItemWithOverride | null> {
    ...
    const result: ItemWithOverride = {
      ...
      sku: item.sku as string,
        type: item.type as import('@synculariti/types').ItemType,
  ...
  async findBySku(sku: string, restaurantId: RestaurantId): Promise<Item | null> {
    const item = await this.db
      ...
      .executeTakeFirst();
    if (!item) return null;
    return this.mapItemRecord(item as unknown as Record<string, unknown>);
  }
  ...
  async listCategories(restaurantId: RestaurantId, franchiseGroupId: string | null, itemType?: string | null): Promise<Category[]> {
    ...
    if (itemType) {
      query = query.where('item_type', '=', itemType as import('@synculariti/types').ItemType);
    }
  ...
  async createCategory(data: CreateCategoryCommand): Promise<Category> {
    const [category] = await this.db
      ...
      .values({
        id: uuidv4() as import('@synculariti/types').CategoryId,
        franchise_group_id: data.franchiseGroupId ? asFranchiseGroupId(data.franchiseGroupId) : null,
        restaurant_id: data.restaurantId ? asRestaurantId(data.restaurantId) : null,
        name: data.name,
  ...
  async updateCategory(categoryId: string, data: UpdateCategoryDto): Promise<Category> {
    ...
    if (data.categoryGroup !== undefined) updateData.category_group = data.categoryGroup;
    updateData.updated_at = new Date().toISOString();
  ...
  async listUomConversions(itemId: ItemId): Promise<UomConversion[]> {
    ...
    return rows.map((row) => ({
      ...
      toUom: row.to_uom,
      multiplierFactor: Number(row.multiplier_factor),
  ...
  private mapItemRecord(row: Record<string, unknown>): Item {
    return {
      ...
      type: row.type as import('@synculariti/types').ItemType,
  ...
  private mapCategoryRecord(row: Record<string, unknown>): Category {
    ...
    return {
      ...
      itemType: row.item_type as import('@synculariti/types').ItemType | null,
  ...
  async deleteCategory(categoryId: string): Promise<void> {
    await this.db
      ...
      .where('id', '=', categoryId as import('@synculariti/types').CategoryId)
      .execute();
  }
  ...
  async generateSku(categoryId: string, restaurantId: RestaurantId | null): Promise<string> {
    ...
    const result = await this.db
      ...
      .where('category_id', '=', categoryId as import('@synculariti/types').CategoryId)
      .executeTakeFirst();
  ...
  async upsertCategory(data: CreateCategoryCommand): Promise<Category> {
    const existing = await this.db
      .selectFrom('categories')
      .selectAll()
      ...
      .where((eb) => {
        ...
        if (data.restaurantId) {
          conditions.push(eb('restaurant_id', '=', asRestaurantId(data.restaurantId)));
        } else {
          conditions.push(eb('restaurant_id', 'is', null));
        }
        if (data.franchiseGroupId) {
          conditions.push(eb('franchise_group_id', '=', asFranchiseGroupId(data.franchiseGroupId)));
        } else {
          conditions.push(eb('franchise_group_id', 'is', null));
        }
    ...
    if (existing) {
      return this.mapCategoryRecord(existing);
    }

    return this.createCategory(data);
  }

  async upsertItem(data: CreateItemCommand): Promise<Item> {
    // Atomic upsert using Kysely's onConflict
    const result = await this.db
      .insertInto('items')
      .values({
        id: uuidv4() as ItemId,
    ...
    if (!result) throw new Error('Failed to upsert item');
    return this.mapItemRecord(result);
  }
}
