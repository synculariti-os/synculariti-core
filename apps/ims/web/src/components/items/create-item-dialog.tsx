...
import { createItemSchema } from '@synculariti/validators';
import { z } from 'zod';
...
export function CreateItemDialog({ isOpen, onClose, onSuccess }: CreateItemDialogProps) {
  ...
  const filteredCategories = categories.filter(
    (c) => !c.itemType || c.itemType === selectedItemType,
  );
    ...
    if (isOpen) {
      const fetchCategories = async () => {
        try {
          ...
          const catRes = await apiClient<{ data: Category[] }>('/items/categories');
              ...
              <div>
                ...
                >
                  ...
                  {filteredCategories.map((c) => (
                    <option key={c.id} value={c.id}>{c.categoryGroup ? `${c.categoryGroup} / ` : ''}{c.name}</option>
                  ))}
                </select>
