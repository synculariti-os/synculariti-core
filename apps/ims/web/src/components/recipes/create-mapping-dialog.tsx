...
import { menuItemMappingSchema } from '@synculariti/validators';
import { z } from 'zod';
import { X, Loader2, Link } from 'lucide-react';
import { apiClient } from '@/lib/api-client';
import { Recipe } from '@synculariti/types';
          ...
          <form id="create-mapping-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              ...
              >
                ...
                {recipes.map(recipe => (
                  <option key={recipe.id} value={recipe.id}>{recipe.recipeName || recipe.id.slice(0, 8)}</option>
                ))}
              </select>
