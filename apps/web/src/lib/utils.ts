...
export async function fetchWithRetry(url: string, options: RequestInit = {}, retries = 3, backoff = 500): Promise<Response> {
  try {
    ...
    if (!response.ok && retries > 0 && response.status >= 500) {
      console.warn(`Fetch failed (${response.status}), retrying in ${backoff}ms...`, { url });
    ...
    if (retries > 0) {
      console.warn(`Fetch threw error, retrying in ${backoff}ms...`, { url, error: getErrorMessage(error) });
      await new Promise(res => setTimeout(res, backoff));
      return fetchWithRetry(url, options, retries - 1, backoff * 2);
    }
    throw error;
