...
import { MenuItemMapping } from '@synculariti/types';
...
export function MappingsTable() {
  ...
  // Client-side search filter + alphabetical sort by POS string
  const filteredMappings = mappings
    .filter(m => searchQuery === '' || m.rawExcelString.toLowerCase().includes(searchQuery.toLowerCase()))
    .sort((a, b) => a.rawExcelString.localeCompare(b.rawExcelString));
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center flex flex-col items-center justify-center text-zinc-500 dark:text-zinc-400">
            ...
            <p className="text-lg font-medium text-zinc-900 dark:text-zinc-100">
              {searchQuery ? 'No mappings match your search' : 'No mappings found'}
            </p>
            <p className="mt-1">
              {searchQuery
                ? 'Try a different search term or clear the filter.'
                : 'Link your POS strings to internal recipes to enable automatic depletion.'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-200/50 dark:border-zinc-800/50 bg-zinc-50/50 dark:bg-zinc-800/20 text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 font-semibold">
                  ...
                  <th className="p-4 px-6 font-medium">Target Recipe</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-200/50 dark:divide-zinc-800/50 text-sm">
                {filteredMappings.map((mapping, idx) => (
                  <tr key={`${mapping.rawExcelString}-${idx}`} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                    <td className="p-4 px-6 text-zinc-900 dark:text-zinc-100 font-medium flex items-center">
                      <FileSpreadsheet className="w-4 h-4 mr-2 text-emerald-500 opacity-70 shrink-0" />
                      {mapping.rawExcelString}
                    </td>
                    <td className="p-4 px-6 text-zinc-500 dark:text-zinc-400 font-mono text-xs">
                      {mapping.targetRecipeName || mapping.recipeId.slice(0, 8)}
                    </td>
                  </tr>
                ))}
              </tbody>
